...
function stageCleanerConversationIds(
  ...
  const unionSql = buildDistinctUnionSql(definitions);
  db.exec(`
    INSERT INTO temp.doctor_cleaner_conversation_ids (conversation_id)
    ${unionSql}
  `);
...
function buildCleanerBackupPath(databasePath: string): string | null {
  ...
  const timestamp = new Date()
    ...
    .replace(/[-:]/g, "")
    .replace(/\.\d{3}Z$/, "Z");
  return join(dirname(trimmed), `${basename(trimmed)}.doctor-cleaners-${timestamp}.bak`);
}
...
export function applyDoctorCleaners(
  ...
  if (!backupPath) {
    return {
      kind: "unavailable",
      reason: "Cleaner apply requires a file-backed SQLite database so Lossless Claw can create a backup first.",
    };
  }
