...
export function runLcmMigrations(
  ...
  runMigrationStep("ensureRollupIndexes", log, () => {
    db.exec(`
      ...
      CREATE INDEX IF NOT EXISTS lcm_rollup_sources_rollup_idx
        ON lcm_rollup_sources (rollup_id);
      CREATE INDEX IF NOT EXISTS lcm_rollup_state_pending_idx
        ON lcm_rollup_state (pending_rebuild) WHERE pending_rebuild = 1;
